...
sub _pre_load {
    ...
    $self->{record_classes} = {
        spectrum     => 'MS::Parser::MzML::Spectrum',
        chromatogram => 'MS::Parser::MzML::Chromatogram',
    };
...
sub fetch_spectrum {
    ...
    my ($self, %args) = @_;
    return $self->fetch_record('spectrum', %args);
...
sub find_by_time {
    ...
    while (defined $ms_level
      && $self->fetch_by_index('spectrum' => $i)->ms_level() != $ms_level) {
        ++$i;
    }
    return $i;
