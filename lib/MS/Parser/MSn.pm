...
sub fetch_spectrum {
    ...
    return MS::Parser::MSn::Spectrum->new($el);
...
MS::Parser::MSn - A simple but complete parser of MS1 and MS2 file formats

=head1 SYNOPSIS

    use MS::Parser::MSn;

    my $p = MS::Parser::MSn->new('run.ms2');

    while (my $s = $p->next_spectrum) {

        # do something - see MS::Parser::MSn::Spectrum
